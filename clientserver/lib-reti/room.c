...
size_t sendPlayer(struct des_room * p, int sd,char padre)
{
	int invio;
	if (padre)
		invio = p->pf[1];
...
int receivePlayer(struct des_room * p, char padre)
{
	int ricezione, sd;
	if (padre)
		ricezione = p->fp[0];
...
size_t startRoom(struct des_room * p)
{
	...
	for (int i = 0; i < MAX_PLAYERS; i++){
		sendPlayer(p,p->players[i],1);
	}
	return 0;
}

size_t joinRoom(int sd, struct des_room * p)
{
